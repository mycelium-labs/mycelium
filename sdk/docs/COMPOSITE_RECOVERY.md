...
returns its durably serialized result, while an unattempted child is admitted
...
The initial scope is straight-line sequential calls. Conditionals, loops,
recursion, dynamic dispatch, nested composites, and unsupported/opaque
boundaries are rejected. Static analysis is preflight assistance, not proof
that arbitrary hidden effects were found. Deterministic local computation may
rerun, but time, randomness, mutable globals, fresh external reads, and other
nondeterministic inputs must not change supported calls or their arguments.

Use a stable host-owned operation ID. A new operation ID means a genuinely new
invocation; a new worker, retry attempt, or parent fence does not. The durable
