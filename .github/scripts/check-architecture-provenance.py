...
def compute_sha256(path: Path) -> str:
    """Compute SHA-256 hash of a file with normalized LF line endings."""
    raw = path.read_bytes()
...
def check_git_ancestor(root: Path, base_commit: str) -> tuple[bool, str]:
    ...
    try:
        ...
        if res.returncode != 0:
            # If the repository is a shallow clone, older commits outside depth 1 cannot be inspected
            ...
            if shallow_check.returncode == 0 and shallow_check.stdout.strip() == "true":
                return (
                    True,
                    f"Repository is a shallow clone; ancestry check for '{base_commit}' bypassed.",
                )
            return (
                False,
...
def validate_provenance(
    ...
    if not manifest_file.exists():
        try:
            rel_display = manifest_file.relative_to(root).as_posix()
        except ValueError:
            rel_display = manifest_file.as_posix()
        msg = f"Provenance manifest not found: {rel_display}"
